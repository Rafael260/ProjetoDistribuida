...
public class ConsumidorAPILeadMe {
    ...
    public static final String BASE_URL = "http://localhost:8081/LeadMeWSMaven/api/leadme/";
    ...
    public Double getMediaAprovacao(Disciplina disciplina) throws ConnectionException{
    	if(disciplina.getMediaAprovacao() != null) {
        	return disciplina.getMediaAprovacao();
        }
        ...
        try {
            ...
            Double media = Double.parseDouble(response);
            disciplina.setMediaAprovacao(media);
            return media;
        } catch (MalformedURLException ex) {
        	throw new ConnectionException(ex.getLocalizedMessage());
        } catch (IOException ex) {
